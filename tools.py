...
class ListIntentsTool(DatasetTool):
    ...
    description: str = (
        "Use this tool to list all unique intents in the dataset. "
...
class CountRowsTool(DatasetTool):
    name: str = "count_rows"
    description: str = (
        "Use this tool to count dataset rows after filtering by exact category, exact intent, "
...
class IntentDistributionTool(DatasetTool):
    ...
    description: str = (
        "Use this tool to calculate the distribution of intents. "
...
class SampleExamplesTool(DatasetTool):
    ...
    description: str = (
        "Use this tool to show concrete examples from the dataset. "
...
class RowsForSummaryTool(DatasetTool):
    ...
    description: str = (
        "Use this tool for unstructured dataset questions that require summarization. "
    ...
    def _run(
        ...
        rows = self.dataset.rows_for_summary(
            category=category,
            intent=intent,
            text_search=text_search,
            limit=limit,
...
class ToolFactory:
    """
    Creates all tools used by the agent.
    """
    ...
    def create_tools(self) -> list[BaseTool]:
        return [
            ListCategoriesTool(dataset=self.dataset),
